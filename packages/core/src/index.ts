...
class Cleo {
  ...
  private constructor() {
    logger.info('File: index.ts 📦, Line: 15, Function: constructor; Initializing Cleo instance');
    this.queueManager = new QueueManager('default');
  }

  static getInstance(): Cleo {
    if (!Cleo.instance) {
      logger.info('File: index.ts 🔄, Line: 22, Function: getInstance; Creating new Cleo instance');
  ...
  configure(config: {
    ...
    try {
      logger.info('File: index.ts ⚙️, Line: 36, Function: configure; Configuring Cleo with redis settings', {
      ...
      if (config.worker) {
        logger.info('File: index.ts 👷, Line: 51, Function: configure; Initializing worker');
        this.worker = new Worker('default', config.worker);
      }

      this.isConfigured = true;
      logger.info('File: index.ts ✅, Line: 56, Function: configure; Cleo configuration complete');
    } catch (error) {
      logger.error('File: index.ts ❌, Line: 58, Function: configure; Configuration failed', { error });
      throw error;
    }
  }
  ...
  getQueueManager(): QueueManager {
    if (!this.isConfigured) {
      logger.error('File: index.ts ⚠️, Line: 64, Function: getQueueManager; Cleo must be configured before using');
